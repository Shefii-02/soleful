        ...
        <div
            ...
            <nav>
                <ol class="flex flex-wrap items-center gap-2">
                    ...
                    <li>
                        <a class="flex items-center gap-3 font-medium" href="{{ route('admin.doctors.index') }}">
                            ...
                            <span class="hover:text-primary">Doctors</span>
                        </a>
                    </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h2 class="text-title-sm2 font-bold text-black dark:text-black">
                    {{ isset($doctor) ? 'Edit doctor' : 'Create a doctor' }}
                </h2>
            </div>
            ...
            <form enctype="multipart/form-data"
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-5 w-full">
                        ...
                        <div x-data="imageHandler('{{ isset($doctor) && $doctor->image ? asset('images/' . $doctor->image) : '' }}')" class="flex gap-15 mt-2">
                            <!-- Image Upload Component -->
                            <template x-if="imageUrl">
                                <div class="relative">
                                    <img :src="imageUrl" class="w-40  object-cover rounded-lg" />
                                    <button type="button"
                            ...
                            <label for="image"
                                class="block cursor-pointer bg-gray-50 border-2 border-dashed border-gray-300 p-4 rounded-lg text-center text-sm text-gray-500 hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400">
                                <span>Click to upload Desktop Image</span><br>
                                <span class="small">Recommended resolution 650x800 with Max size 100kb</span>
                                <small></small>
                    ...
                    <div class="mb-5 w-full">
                        <label class="block text-sm mb-2 font-medium text-black dark:text-dark">Qualifications</label>
                        <textarea  autocomplete="off" name="qualifications" class="form-control">{{ old('qualifications', $doctor->qualifications ?? '') }}</textarea>
                    </div>
                    <div class="mb-5 w-full">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Department</label>
                        <input  autocomplete="off" type="text" name="department" class="form-control"
                            value="{{ old('name', $doctor->name ?? '') }}" />
                    </div>
                    <div class="mb-5 w-full">
                        <label class="block text-sm mb-2 font-medium text-black dark:text-dark">Description</label>
                    ...
                    <div class="mb-5 w-1/4">
                        ...
                        <select class="form-control mt-2" name="display_order" required>
                            @for ($i = 1; $i < 20; $i++)
                                <option {{ isset($doctor) && $doctor->display_order == $i ? 'selected' : '' }}
                                    value="{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                    </div>

                    <div x-data="{ slots: [{ day: '', startTime: '', endTime: '' }] }" class="mb-5 w-1/2">
                        <label class="block text-sm mb-2 font-medium text-black dark:text-dark">Availability Slots</label>
                        @if (isset($doctor) && $doctor->availabilities)
                            @foreach ($doctor->availabilities ?? [] as $key => $availabiltSlot)
                                <div class="flex gap-4 items-center mb-3" id="100{{ $key }}">
                                    <!-- Day Selection -->
                                    <div>
                                        <label for="day"
                                            class="block text-sm mb-2 font-medium text-gray-700">Day</label>
                                        <select name="slots[100{{ $key }}][day]" required
                                            class="block w-full border-gray-300 form-control rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                            <option {{ $availabiltSlot->day == '' ? 'selected' : '' }} value="">
                                                Select
                                                Day</option>
                                            <option {{ $availabiltSlot->day == '0' ? 'selected' : '' }} value="0">
                                                Sunday
                                            </option>
                                            <option {{ $availabiltSlot->day == '1' ? 'selected' : '' }} value="1">
                                                Monday
                                            </option>
                                            <option {{ $availabiltSlot->day == '2' ? 'selected' : '' }} value="2">
                                                Tuesday
                                            </option>
                                            <option {{ $availabiltSlot->day == '3' ? 'selected' : '' }} value="3">
                                                Wednesday</option>
                                            <option {{ $availabiltSlot->day == '4' ? 'selected' : '' }} value="4">
                                                Thursday</option>
                                            <option {{ $availabiltSlot->day == '5' ? 'selected' : '' }} value="5">
                                                Friday
                                            </option>
                                            <option {{ $availabiltSlot->day == '6' ? 'selected' : '' }} value="6">
                                                Saturday</option>
                                        </select>
                                    </div>
                                    ...
                                    <!-- Start Time -->
                                    <div>
                                        <label for="start-time" class="block text-sm mb-2 font-medium text-gray-700">Start
                                            Time</label>
                                        <input type="time" value="{{ $availabiltSlot->start_time }}"
                                            name="slots[100{{ $key }}][start_time]" required
                                            class="block w-full border-gray-300 form-control rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>

                                    <!-- End Time -->
                                    <div>
                                        <label for="end-time" class="block text-sm mb-2 font-medium text-gray-700">End
                                            Time</label>
                                        <input type="time" value="{{ $availabiltSlot->end_time }}"
                                            name="slots[100{{ $key }}][end_time]" required
                                            class="block w-full border-gray-300 rounded-md form-control shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>

                                    <!-- Remove Button -->
                                    <button type="button" @click="removeFieldParentDiv(100{{ $key }})"
                                        class="bg-danger float-end btn-sm text-sm btn text-light">
                                        Remove
                                    </button>
                                </div>
                            @endforeach
                        ...
                        <template x-for="(slot, index) in slots" :key="index">
                            ...
                            <div class="flex gap-4 items-center mb-3">
                                <!-- Day Selection -->
                                <div>
                                    <label for="day" class="block text-sm mb-2 font-medium text-gray-700">Day</label>
                                    <select x-model="slot.day" :name="`slots[${index}][day]`" required
                                        class="block w-full border-gray-300 form-control rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                <div>
                                    <label for="start-time" class="block text-sm mb-2 font-medium text-gray-700">Start
                                        Time</label>
                                    <input type="time" x-model="slot.startTime" :name="`slots[${index}][start_time]`"
                                        required
                                        class="block w-full border-gray-300 form-control rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                </div>
                                ...
                                <div>
                                    <label for="end-time" class="block text-sm mb-2 font-medium text-gray-700">End
                                        Time</label>
                                    <input type="time" x-model="slot.endTime" :name="`slots[${index}][end_time]`"
                                        required
                                        class="block w-full border-gray-300 rounded-md form-control shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                </div>
                        ...
                        <!-- Add More Button -->
                        <button type="button" @click="slots.push({ day: '', startTime: '', endTime: '' })"
                            class="bg-dark float-end btn-sm text-white me-3 rounded-md hover:bg-blue-600">
                            Add More
                        </button>
                    </div>
                    ...
                    <!-- Status Toggle -->
                    <div class="mb-5">
                        <label class="block text-sm mb-2 font-medium text-black dark:text-dark">Status</label>
                        <div class="form-group mb-3 col-md-4">
                            <label class="form-check form-switch">
                                <input class="form-check-input" name="status" type="checkbox" value="1"
                                    id="status"
                                    {{ old('status', isset($doctor) && $doctor->status ? 'checked' : '') }}>
