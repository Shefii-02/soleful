            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div class="border-b border-stroke px-4 pb-2 dark:border-strokedark md:px-6 xl:px-7.5">
                    <div class="flex justify-between items-center gap-x-6">
                        ...
                        <div class="w-1/12 text-center"><span class="font-medium">Image</span></div>
                        <div class="w-2/12 text-center"><span class="font-medium">Name</span></div>
                        <div class="w-2/12 text-center"><span class="font-medium">Department</span></div>
                        ...
                        @forelse ($doctors as $key => $doctor)
                            <div class="flex justify-between items-center gap-x-6">
                                ...
                                <div class="w-1/12 flex justify-center">
                                    <img src="{{ asset('images/' . $doctor->image) }}" class="w-50 rounded-5">
                                </div>
                                <div class="w-2/12 flex justify-center">
                                    <span class="font-medium">{{ $doctor->name }}</span>
                                </div>
                                <div class="w-2/12 text-center">
                                    <span class="font-medium">{{ $doctor->department }}</span>
                                </div>
                                <div class="w-1/12 text-center">
                                    <span class="font-medium">{{ $doctor->display_order }}</span>
                                </div>
                                <div class="w-2/12 text-center">
                                    <span
                                ...
                                <div class="w-2/12 text-center">
                                    <span
                                        class="inline-block text-capitalize rounded px-2.5 py-0.5 text-sm font-medium text-white {{ $doctor->status === 0 ? 'bg-danger' : 'bg-success' }}">
                                        {{ $doctor->status_html }}
                                    </span>
                                </div>
                                <div class=" w-1/12 text-end">
                                    <div class=" btn-group">
                                        ...
                                        <form method="POST" id="form_{{ $doctor->id }}"
                                            action="{{ route('admin.doctors.destroy', $doctor->id) }}">@csrf
                                            @method('DELETE')</form>
                                        <button form="form_{{ $doctor->id }}" type="button"
