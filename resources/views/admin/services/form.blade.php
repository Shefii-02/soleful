    ...
    <div
        ...
        <nav>
            <ol class="flex flex-wrap items-center gap-2">
                ...
                <li>
                    <a class="flex items-center gap-3 font-medium" href="{{ route('admin.services.index') }}">
                      ...
                      <span class="hover:text-primary">Services</span>
                    </a>
                  </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h2 class="text-title-sm2 font-bold text-black dark:text-black">
                    {{ isset($service) ? 'Edit service' : 'Create a service' }}
                </h2>
            </div>
            ...
            <form enctype="multipart/form-data"
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-5 w-full">
                        ...
                        <div x-data="imageHandler('{{ isset($service) && $service->image ? asset('images/' . $service->image) : '' }}')" class="mt-2 gap-15 flex">
                            ...
                            <label for="image"
                                ...
                                <span class="small">Recommended resolution 860*1000 with Max size 100kb</span>
                                <small></small>
                    ...
                    <div class="mb-5 w-1/2">
                        ...
                        <input type="text" autocomplete="off" name="name" class="form-control" value="{{ old('name', $service->name ?? '') }}" />
                    </div>
                  
                    <div class="mb-5 w-full">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Description</label>
                        <input type="text" name="description" class="form-control tinyeditor" value="{{ old('description', $service->description ?? '') }}" />
                    </div>
                    <!-- Display Order -->
                    <div class="mb-5 w-1/4">
                        ...
                        <select class="form-control mt-2" name="display_order" required>
                            @for ($i = 1; $i < 10; $i++)
                                <option {{ isset($service) && $service->display_order == $i ? 'selected' : '' }}
                                    value="{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                        ...
                        <div class="form-group mb-3 col-md-4">
                            <label class="form-check form-switch">
                                <input class="form-check-input" name="status" type="checkbox" value="1" id="status"
                                    {{ old('status', isset($service) && $service->status ? 'checked' : '') }}>
