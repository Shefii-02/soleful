                        ...
                        @forelse ($packages as $key => $package)
                            <div class="flex justify-between items-center gap-x-6">
                                ...
                                <div class="w-1/12 flex justify-center">
                                    <img src="{{ asset('images/' . $package->image) }}" class="w-50 rounded-5">
                                </div>
                                <div class="w-2/12 flex justify-center">
                                    <span class="font-medium">{{ $package->name }}</span>
                                </div>
                                <div class="w-1/12 text-center">
                                    <span class="font-medium">{{ $package->display_order }}</span>
                                </div>
                                ...
                                <div class="w-2/12 text-center">
                                    <span
                                        class="inline-block text-capitalize rounded px-2.5 py-0.5 text-sm font-medium text-white {{ $package->status === 0 ? 'bg-danger' : 'bg-success' }}">
                                        {{ $package->status_html }}
                                    </span>
                                </div>
                                <div class=" w-1/12 text-end">
                                    <div class=" btn-group">
                                        ...
                                        <form method="POST" id="form_{{ $package->id }}"
                                            action="{{ route('admin.packages.destroy', $package->id) }}">@csrf
                                            @method('DELETE')</form>
                                        <button form="form_{{ $package->id }}" type="button"
