...
<div class="py-4">
    ...
    <div>
            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div class="border-b border-stroke px-4 pb-2 dark:border-strokedark md:px-6 xl:px-7.5">
                    <div class="flex justify-between items-center gap-x-6">
                        ...
                        <div class="w-4/12 text-left"><span class="font-medium">Appoinment for</span></div>
                        <div class="w-2/12 text-left"><span class="font-medium">Message</span></div>
                        <div class="w-1/12 text-center"><span class="font-medium">Status</span></div>
                        <div class="w-1/12 text-center"><span class="font-medium">Actions</span></div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col gap-y-4">
                        @foreach ($appointments as $appointment)
                            <div class="flex justify-between items-center gap-x-6">
                                ...
                                <div class="w-2/12 text-left">
                                    <span class="font-medium">{{ $appointment->mobile }}</span>
                                </div>
                                <div class="w-4/12 text-left">
                          
                                    <span>{{ $appointment->doctor_id ? '' : 'Service' }}</span>
                                ...
                                <div class="w-1/12 text-center">
                                    <span
                                        class="inline-block text-capitalize rounded px-2.5 py-0.5 text-sm font-medium text-white {{ $appointment->status === 0 ? 'bg-red' : 'bg-success' }}">
                                        {{ $appointment->status == 0 ? 'Unread' : 'attended' }}
                                    </span>
                                </div>
                                <div class="w-1/12 text-center">
                                    <div class="flex justify-center">
                                        <button data-id="{{ $appointment->id }}"
                                            class="open-appointment-modal block hover:text-meta-1">
        ...
        <!-- Modal -->
        <div id="appointment-modal" class="hidden fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('.open-appointment-modal').forEach(button => {
                button.addEventListener('click', async (e) => {
                    const appointmentId = e.target.closest('button').getAttribute('data-id');
