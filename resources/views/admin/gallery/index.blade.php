                        ...
                        @forelse ($galleries as $key => $gallery)
                            <div class="flex justify-between items-center gap-x-6">
                                ...
                                <div class="w-2/12 flex justify-center">
                                    <img src="{{ asset('images/' . $gallery->image) }}" class="w-50 rounded-5">
                                </div>
                                <div class="w-1/12 text-center">
                                    <span class="font-medium">{{ $gallery->display_order }}</span>
                                </div>
                                <div class="w-2/12 text-center">
                                    <span
                                        class="font-medium px-2.5 py-0.5 text-sm font-medium inline-block text-capitalize rounded">
                                        {{ date('M d, Y', strtotime($gallery->created_at)) }}
                                    </span>
                                </div>
                                <div class="w-2/12 text-center">
                                    <span
                                        class="inline-block text-capitalize rounded px-2.5 py-0.5 text-sm font-medium text-white {{ $gallery->status === 0 ? 'bg-danger' : 'bg-success' }}">
                                        {{ $gallery->status_html }}
                                    </span>
                                </div>
                                <div class=" w-2/12 text-end">
                                    <div class=" btn-group">
                                        ...
                                        <form method="POST" id="form_{{ $gallery->id }}"
                                            action="{{ route('admin.gallery.destroy', $gallery->id) }}">@csrf
                                            @method('DELETE')</form>
                                        <button form="form_{{ $gallery->id }}" type="button"
